...
class QueryDslTest {
	...
	void predicateExecutor() {
		assertThat(repository.findAll(employee.department.eq("IT").and(employee.salary.goe(6000))))
				.extracting(Employee::getName).containsExactly("Alice");

		assertThat(repository.findAll(employee.active.isTrue(), Sort.by("salary").descending()))
				.extracting(Employee::getName).containsExactly("Alice", "Carol", "Bob");

		assertThat(repository.count(employee.department.eq("HR"))).isEqualTo(2);
		assertThat(repository.exists(employee.name.startsWith("Da"))).isTrue();
	}
