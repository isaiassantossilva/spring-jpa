...
class StreamLimitScrollTest {
	...
	void streamResults() {
		...
		try (Stream<Employee> stream = repository.streamActive()) {
	...
	void dynamicLimit() {
		assertThat(repository.findByActiveTrueOrderBySalaryDesc(Limit.of(2)))
				.extracting(Employee::getName).containsExactly("Alice", "Carol");

		assertThat(repository.findByActiveTrueOrderBySalaryDesc(Limit.unlimited())).hasSize(4);
	}

	@Test
	@DisplayName("Scroll API com keyset: continua de onde parou sem OFFSET")
	void keysetScrolling() {
		Window<Employee> first = repository
		...
		Window<Employee> second = repository
