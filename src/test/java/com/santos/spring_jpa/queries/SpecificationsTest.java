...
class SpecificationsTest {
	...
	void singleSpecification() {
		assertThat(this.repository.findAll(hasDepartment("IT"))).hasSize(2);
	}
	...
	void combinedSpecifications() {
		...
		assertThat(this.repository.findAll(itWellPaid))
		...
		assertThat(this.repository.findAll(itOrHrActive)).hasSize(3);
	}

	@Test
	@DisplayName("specification com Sort e count")
	void specificationWithSortAndCount() {
		assertThat(this.repository.findAll(isActive(), Sort.by("salary").descending()))
				.extracting(Employee::getName).containsExactly("Alice", "Carol", "Bob");

		assertThat(this.repository.count(nameLike("o"))).isEqualTo(2); // Bob, Carol
	}
}
